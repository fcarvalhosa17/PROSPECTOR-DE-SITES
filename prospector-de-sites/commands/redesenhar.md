...
2. **Direção de arte**: antes de escrever HTML, escreva o conceito da página (3-5 linhas, conforme a seção "Direção de arte" da skill) e confira que ele é DIFERENTE dos outros clientes do lote — fontes e composição não podem se repetir entre clientes. Se as skills `ui-ux-pro-max` e/ou `frontend-design` estiverem instaladas, consulte-as neste passo (paletas, pares de fontes, estilos e regras anti-estética-de-IA) para fundamentar o conceito.
3. **Copy**: se o agente `copy-engine` estiver disponível, delegue a ele a escrita/revisão de toda a copy da página (headlines, seções, CTAs, microcopy) passando o conteúdo REAL extraído + as regras de copywriting da skill `redesign-premium` (nada inventado). Sem o agente, escreva a copy você mesmo seguindo a skill.
4. **Redesign**: aplique a skill `redesign-premium` na íntegra. Regra de ouro: NADA inventado — é uma nova versão da página do cliente, não uma página nova. O logo original e as fotos originais DEVEM aparecer na página nova (se o cliente não tem site/logo, use composição tipográfica — nunca invente logo).
5. **SEO on-page**: o site novo vai SUBSTITUIR o antigo — não pode perder ranking. Garanta em cada página: `<title>` e meta description com nome + especialidade + cidade, hierarquia de headings correta (1 só `h1`), `alt` descritivo em todas as fotos, e JSON-LD `LocalBusiness` (ou subtipo: `Dentist`, `Attorney`, `HairSalon`…) com os dados REAIS do lead (nome, endereço, telefone, horários, nota do Google). Se o agente `seo` estiver disponível, delegue a ele essa revisão (é especialista em migração sem perda de ranking); senão, aplique você mesmo.
6. **Salvar** na pasta conectada, com o nome do cliente no arquivo para fácil identificação:
