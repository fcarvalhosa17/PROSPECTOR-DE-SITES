...
- **Aba Claude (terminal embutido)**: se houver um `ttyd.exe` (baixado do GitHub, colocado na pasta conectada ou no PATH), o servidor sobe o ttyd em `127.0.0.1:7681` servindo `cmd /k claude` e a aba "Claude" embute esse terminal por iframe — Claude Code interativo de verdade (aprova permissões, streaming, roda git/vercel). É o caminho para tarefas que precisam de rede ou aprovação. Rota `/api/terminal` informa se o ttyd está instalado/no ar; sem ttyd, a aba mostra as instruções de download.
- **Prompt no Comparador**: abaixo do antes/depois há um campo de prompt — o pedido vai pro `/api/chat` (headless, `--allowedTools` só de arquivo local, sem rede, para não travar) com o arquivo do site no contexto; o iframe "Depois" recarrega ao terminar e o histórico persiste na tabela `conversas` por slug (rotas `/api/conversas`). Edições que precisam de rede vão pela aba Claude (terminal).
